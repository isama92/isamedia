...
impl Browse {
    ...
    pub fn on_key(&mut self, key: KeyEvent) -> Option<BrowseAction> {
        ...
        match key.code {
            // Before the paging keys: `d` means delete on the (list-less)
            // episode detail and page-down everywhere else.
            KeyCode::Char('d') if matches!(self.level, Level::EpisodeDetail { .. }) => {
            ...
            KeyCode::Char('x') => {
                if matches!(self.level, Level::Search { .. })
                    && self
                        .selected_release()
